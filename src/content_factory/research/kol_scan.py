...
class KolScanner:
    """High-level wrapper for industry KOL scan.

    Example:
        >>> from content_factory.research import KolScanner
        >>> scanner = KolScanner(topic="光通信", slug="glass-bridge-cpo")
        >>> scanner.run()
    """
    ...
    def run(self) -> dict:
        ...
        Returns:
            Scan results dict.
        """
        # Note: This is a thin wrapper. Full implementation requires
        # web_search integration which is agent-side. The actual scan
        # is typically invoked through the content-factory pipeline.
        from pathlib import Path

        output_dir = Path(f"drafts/raw/{self.slug}")
        ...
        return {
            ...
            "status": "ready",
            "note": "Invoke via content-factory agent pipeline for full scan.",
        }
