...
class TopicScorer:
    """High-level wrapper for topic scoring (MOC match + freshness + depth).

    Example:
        >>> from content_factory.pipeline import TopicScorer
        >>> scorer = TopicScorer()
        >>> score = scorer.score(title="光纤涨价 178%", moc="MOC-AI算力")
    """

    def __init__(self) -> None:
        pass

    def score(self, title: str, moc: str = "", **kwargs) -> float:
        """Score a topic.

        Returns a score from 0-100.
        """
        # Stub — full implementation lives in the module-level main()
        return 50.0
