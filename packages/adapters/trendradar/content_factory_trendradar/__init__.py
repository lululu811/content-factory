...
class TrendRadarDataSource(DataSourceProvider):
    ...
    def __init__(self, db_dir: str | None = None) -> None:
        self.db_dir = Path(db_dir or os.environ.get("CF_TRENDRADAR_DB_DIR", _DEFAULT_DB_DIR))
    ...
    async def fetch_rss(self, feed_id: str, days: int = 1, limit: int = 50) -> list[dict[str, Any]]:
        """获取指定 RSS 源最近 N 天的条目"""
        items = []
    ...
    async def daily_summary(self, day: date | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            cur = conn.execute(
                "SELECT title, url, feed_id, published_at FROM rss_items ORDER BY published_at DESC"
            )
            for row in cur.fetchall():
                by_feed[row[2]].append({"title": row[0], "url": row[1], "published_at": row[3]})
            ...
            return {
                ...
                "feeds": sorted(feeds_summary, key=lambda x: x["count"], reverse=True),
            }
        finally:
            conn.close()
    ...
    def _score(item: dict, keywords: list[str]) -> float:
        ...
        try:
            ...
            age_hours = (datetime.now(dt.tzinfo) - dt).total_seconds() / 3600 if dt.tzinfo else 0
            if age_hours < 6:
                s += 3.0
            elif age_hours < 24:
                s += 1.5
